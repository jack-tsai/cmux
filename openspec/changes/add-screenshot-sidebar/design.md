...
### Drag-out：`NSItemProvider` 提供雙型別 + 私有標籤限制目標

實作 `Draggable` behavior 透過 `NSItemProvider` 註冊三個 type identifier：
...
- `com.cmux.screenshot-panel-drag`（private UTI）：標籤 drag source 為 screenshot panel；非 terminal 的 drop handler（File Explorer、Sessions、Finder）看到這個 identifier 就拒絕 drop，OS 顯示 "not allowed" badge。

相較「等同 Finder drag」的替代方案更保守：v1 只允許拖到 cmux 自己的 terminal。這避免使用者誤把 `~/Pictures/螢幕載圖` 的原檔拖到 Finder 誤 move、或拖到 File Explorer 期待某種 import 但其實沒有。若將來要放寬改為「等同 Finder」，只需要移除該 private UTI 的拒絕邏輯即可。
...
- Preview（4:3 max ~280 px 寬）：預設走 `QLThumbnailGenerator` 請求 1024 pt longest-edge，不 decode 原圖；只有 ≤ 512 KB 的小檔才 fallback 直讀 `NSImage(contentsOf:)` 以保留銳利度。這樣把 100 MB PNG 這類 edge case 的 memory footprint 控制在 ~50 MB 以內。
...
- **SSH workspace 該不該禁用 paste？** 本機截圖 scp 到遠端再 paste reference，大截圖（10+ MB）上傳會明顯卡幾秒。**已決**：不禁用；上傳超過 500 ms 時 gallery cell 顯示 spinner overlay，terminal scrollback 不寫任何 `uploading…` 文字，避免污染 command 輸出。見 `specs/screenshot-terminal-paste` 的 `SSH upload progress feedback on gallery cell` requirement。
