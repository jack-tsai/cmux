...
- **Never derive theme palettes as computed properties.** `NSColor.blended(withFraction:of:)` (and `darken` / `lighten` that sit on the same path) round-trips through ColorSync on every read, which costs 50–100 µs and pegs ColorSync on the main thread when a view (for example one commit row per LazyVStack slot) reads the derived palette per render. Cache palettes in `@State` (or an equivalent value type stored via `@StateObject`) and refresh them only on the ghostty `com.cmuxterm.themes.reload-config` notification. See `GitGraphTheme.make(from:)` and the comment above `@State private var theme` in `Sources/Panels/GitGraphPanelView.swift` for the enforcement pattern.
- **`git for-each-ref` format language ≠ `git log` format language.** `%x00` only expands to a NUL byte inside `git log` / `git show` / `git stash list --format=`. Inside `git for-each-ref` it stays as the literal four characters `%x00`, and passing a Swift string with an embedded `\u{00}` as a Process argument aborts Foundation's argv conversion. When you need a separator for a `for-each-ref --format=...` invocation, use Tab (`\t`); git ref names cannot contain Tab and SHAs are hex-only so there is no false-positive risk.
